...
<body class="bg-gray-100">
    <div class="max-w-2xl mx-auto p-6 bg-white shadow-md rounded-lg mt-10">
        <h1 class="text-2xl font-semibold text-gray-700 mb-6">Edit Blogs</h1>
        @if ($errors->any())
        @endif
        ...
        <form action=" {{ route('blog.update', ['id' => $blog->id]) }} " method="POST" class="space-y-6">
            ...
            <div class="">
                ...
                <textarea type="text" name="description" id="description"
                    ...
                    @enderror mt-1 block w-full border border-gray-300 rounded-md shadow-sm px-4 py-2 focus:ring-4 focus:ring-blue-500 focus:border-blue-500">{{ $blog->descripsi }}</textarea>
            </div>
            @error('description')
                <div class=" mt-1 p-4 mb-4 text-sm text-red-800 rounded-lg bg-red-50"> {{ $message }} </div>
            @enderror
            ...
            <ul
                ...
                @foreach ($tags as $t)
                    ...
                    <div class="flex items-center ps-3">
                        <input id="checkbox-{{ $t->id }}" type="checkbox" value="{{ $t->id }}" name="tags[]"  {{  $blog->tags->contains($t->id) ? 'checked' : ''  }}
